...
namespace KhoDuoc
{
    public partial class FrmDoiMatKhau : Form
    {
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("select count (*) from [dbo].[DUOC_KC_tab.TAIKHOAN] where TenDangNhap=N'" + txtTenDangNhap.Text + "'and MatKhau=N'" + txtMatKhauMoi.Text + "' ", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
            errorProvider1.Clear();
            if (dt.Rows[0][0].ToString() == "0")
            {
                ...
                if (txtMatKhauMoi.Text == txtNhapLaiMatKhau.Text)
                {
                    ...
                    SqlDataAdapter dal = new SqlDataAdapter("update [dbo].[DUOC_KC_tab.TAIKHOAN] set MatKhau=N'" + txtMatKhauMoi.Text + "' where TenDangNhap=N'" + txtTenDangNhap.Text + "' and MatKhau=N'" + txtMatKhauCu.Text + "' ", con);
                    DataTable tb = new DataTable();
                    dal.Fill(tb);
                    MessageBox.Show("đổi mật khẩu thành công");
                    this.Close();
                ...
                {
                    errorProvider1.SetError(txtTenDangNhap, "Vui lòng điền tên đăng nhập");
                    errorProvider1.SetError(txtMatKhauMoi, "Vui lòng nhập mật khẩu mới");
                    errorProvider1.SetError(txtNhapLaiMatKhau, "Vui lòng xác nhận mật khẩu");
        ...
        private void chkpass1_CheckedChanged_2(object sender, EventArgs e)
        {
            if (chkpass1.Checked)
            {
